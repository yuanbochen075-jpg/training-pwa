...
(function () {
  ...
  const COACH_OVERRIDES = {
    ...
    '2026-08-24': {
      ...
      note: '昨晚自我训练下肢量偏大+睡眠质量一般：今天轻恢复，不测试不冲刺。弹跳基线已顺延至8/27（周四）。如实在想补测：摸高/CMJ/立定跳各3次取最好即可，别加量。',
      foods: [
        { when: '全天', items: '番茄、猕猴桃、核桃、石榴、蓝莓' },
        { when: '晚餐', items: '正常蛋白质餐（鸡胸/鱼/鸡蛋）+ 一小把坚果' }
      ]
    },
    // 2026-08-27：大雨改练力量（见 extra-data.js 加练记录），弹跳基线顺延至8/29
    '2026-08-27': {
      type: 'rest', title: '雨天·力量训练（见加练记录）', venue: '—', duration: '—', warmup: '',
      main: [],
      note: '大雨未测弹跳基线：今日力量训练见「加练记录」。深蹲金字塔（50→80kg）后腿有疲劳，弹跳基线顺延至8/29（周六）测。1000m 对短跑价值最低，本周先放弃；想测可放到之后轻松日。',
      foods: [
        { when: '全天', items: '番茄、猕猴桃、核桃、石榴、蓝莓' },
        { when: '晚餐', items: '正常蛋白质餐（鸡胸/鱼/鸡蛋）+ 一小把坚果' }
      ]
    },
    // 2026-08-25：下雨未测速，改练上身（见 extra-data.js 加练记录）；测试②顺延
    '2026-08-25': {
      type: 'rest', title: '雨天·短跑计时顺延', venue: '—', duration: '—', warmup: '',
      main: [],
      note: '因下雨未测速：测试② 30/60/100m 顺延至 8/26（周三）。今日上身力量训练见「加练记录」。若 8/26 仍下雨可再顺延，测试前让腿保持新鲜。',
      foods: [
        { when: '全天', items: '番茄、猕猴桃、核桃、石榴、蓝莓' },
        { when: '晚餐', items: '正常蛋白质餐（鸡胸/鱼/鸡蛋）+ 一小把坚果' }
      ]
    },
    // 2026-08-26：补测 测试② 短跑计时（原8/25顺延，替换1000m；1000m移到8/29）
    '2026-08-26': {
      type: 'test', title: '测试②（补）短跑计时', venue: '田径场', duration: '50min',
      warmup: '动态热身15min + 2×30m加速',
      main: [
        { name: '30m', sets: 2, reps: '全力计时，取最好' },
        { name: '60m', sets: 2, reps: '全力计时，取最好' },
      ...
      note: '补测基线（原8/25因雨顺延）。若仍下雨且场地湿滑，宁可不测也别硬跑。1000m已移到8/29（周六）。'
    },
    // 2026-08-28：持续降雨，户外短跑计时无法进行 -> 恢复日；弹跳基线明早室内测
    '2026-08-28': {
      type: 'rest', title: '雨天·恢复日（短跑计时顺延）', venue: '—', duration: '—', warmup: '',
      main: [],
      note: '本周持续降雨，户外短跑计时无法进行：30/60/100m 顺延至下周第一个不下雨的训练日（8/31 周一速度日优先），不硬凑、不冒险。今天休息+拉伸，让 8/27 深蹲后的腿恢复；明早 8/29 在家室内测弹跳基线。',
      foods: [
        { when: '全天', items: '番茄、猕猴桃、核桃、石榴、蓝莓' },
        { when: '晚餐', items: '正常蛋白质餐（鸡胸/鱼/鸡蛋）+ 一小把坚果' }
      ]
    },
    // 2026-08-29：赶高铁回家，无训练；弹跳基线顺延至8/30
    '2026-08-29': {
      type: 'rest', title: '出行日·赶高铁回家', venue: '—', duration: '—', warmup: '',
      main: [],
      note: '赶高铁回家，无训练。弹跳基线顺延至8/30（周日）在家测。',
      foods: [
        { when: '全天', items: '番茄、猕猴桃、核桃、石榴、蓝莓' },
        { when: '晚餐', items: '正常蛋白质餐（鸡胸/鱼/鸡蛋）+ 一小把坚果' }
      ]
    },
    // 2026-08-30：基线测试推迟（回校后W3与短跑计时一起补）
    '2026-08-30': {
      type: 'rest', title: '恢复日（基线测试推迟）', venue: '—', duration: '—', warmup: '',
      main: [],
      note: '弹跳基线推迟至回校后（W3 第一个新鲜日，与短跑计时一起测）。今日轻恢复+拉伸，适应新环境。',
      foods: [
        { when: '全天', items: '番茄、猕猴桃、核桃、石榴、蓝莓' },
        { when: '晚餐', items: '正常蛋白质餐（鸡胸/鱼/鸡蛋）+ 一小把坚果' }
      ]
    ...
    '2026-08-31': {
      type: 'aerobic', title: '全身循环①·室内版 + 有氧（雨天）', venue: '家', duration: '40-50min',
      warmup: '动态热身5-8min',
      main: [
        ...
        { name: '循环·两头起（下背贴地）', sets: '2-3轮', reps: '20次', note: '替代引体，别甩腰' },
        { name: '循环·提踵（慢上慢下）', sets: '2-3轮', reps: '30次' },
        { name: '循环·俄罗斯转体', sets: '2-3轮', reps: '40s' },
        { name: '循环·平板支撑', sets: '2-3轮', reps: '60s' },
        { name: '室内有氧替代', sets: 1, reps: '原地慢跑/踏步20-30min', pace: '心率130-140', note: '有跳绳/楼梯也可用' }
      ],
      note: '雨天室内版：引体→两头起、Pogo→提踵、开合跳→平板、登山跑→俄转。慢跑改室内原地有氧，心率说话。雨停可回户外版。'
    },
    '2026-09-01': {
